...
export async function GET(
  ...
  if (payment.status !== "paid") {
    return NextResponse.json(
      { error: "Receipt tersedia setelah pembayaran lunas" },
      { status: 409 },
    );
  }

  const pdf = paymentPdf("receipt", payment, {
  ...
  return new Response(new Uint8Array(pdf), {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="Receipt-${payment.orderId}.pdf"`,
